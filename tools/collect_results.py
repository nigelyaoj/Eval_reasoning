...
def process_file(args):
    ...
    metrics_file, cal_tokens = args
    try:
        # Get model and benchmark names
        model_name_norm = get_model_name(metrics_file)
        ...
        if cal_tokens:
            ...
            tokenizer = get_tokenizer(model_tokenizers[model_name_norm])
...
def collect_results(base_dir, args):
    ...
    metrics_files = glob.glob(f"{base_dir}/**/test_{args.prompt_type}*metrics.json", recursive=True)
    
    # Create arguments for parallel processing
    process_args = [(f, args.cal_tokens) for f in metrics_files]
...
def main(args):
    ...
    results = collect_results(base_dir, args)
    ...
    print("\nResults summary:")
    show_cols = [col for col in df.columns if (not "_tokens" in col) ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_dir", type=str, default="Qwen")
    parser.add_argument("--output_path", type=str, default="Qwen/results.csv")
    ...
    parser.add_argument("--benchmarks", type=str, 
                    #    default="gsm8k,math500,minerva_math,gaokao2023en,olympiadbench,college_math,aime24,amc23",
                       default="gsm8k,math500,olympiadbench,college_math",
                    #    default="gsm8k,math500,minerva_math,olympiadbench,aime24,amc23", # simple RL report                  
                       help="Comma-separated list of benchmarks to include")
    parser.add_argument("--cal_tokens", action="store_true")
